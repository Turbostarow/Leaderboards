...
// ── Rank lists (ascending order — higher index = better rank) ─
...
// Longest-match alternations (multi-word ranks matched first)
...
export function parseDate(raw) {
  ...
  const discordTs = raw.trim().match(/^<t:(\d+)(?::[A-Za-z])?>/);
...
// ── Strip Discord mention/emoji formatting ────────────────────
// Converts <@123456> → '' and <:emoji:123> → 'emoji'
// so the regex sees clean text

function stripDiscordFormatting(s) {
  return s
    .replace(/<@!?\d+>/g, '')          // user mentions
    .replace(/<:([\w]+):\d+>/g, '$1')  // custom emoji → name
...
export function parseMarvelRivals(content) {
  const body = stripDiscordFormatting(content.replace(/^\s*LB_UPDATE_MR:\s*/i, '').trim());

  const playerMatch = body.match(/^@?(.+?)\s+(\S+)\s+(.+)$/);
  if (!playerMatch) {
    console.warn(`[parser] MR: could not extract name/role/rest from: "${body.slice(0, 100)}"`);
    return null;
  }
  ...
  if (!rankCurrent || !rankPeak) {
    console.warn(`[parser] MR: unknown rank — current:"${rCur}" peak:"${rPeak}"`);
    return null;
  }
  ...
  return { game: 'MARVEL_RIVALS', playerName, role, rankCurrent, tierCurrent, rankPeak, tierPeak, date: parseDate(dateRaw), dateRaw: dateRaw.trim() };
}
...
export function parseOverwatch(content) {
  const body = stripDiscordFormatting(content.replace(/^\s*LB_UPDATE_OW:\s*/i, '').trim());

  const playerMatch = body.match(/^@?(.+?)\s+(\S+)\s+(.+)$/);
  if (!playerMatch) {
    console.warn(`[parser] OW: could not extract name/role/rest from: "${body.slice(0, 100)}"`);
    return null;
  }
  ...
  if (!rankCurrent || !rankPeak) {
    console.warn(`[parser] OW: unknown rank — current:"${rCur}" peak:"${rPeak}"`);
    return null;
  }
  ...
  return { game: 'OVERWATCH', playerName, role, rankCurrent, tierCurrent, currentValue, rankPeak, tierPeak, peakValue, date: parseDate(dateRaw), dateRaw: dateRaw.trim() };
}
...
export function parseDeadlock(content) {
  const body = stripDiscordFormatting(content.replace(/^\s*LB_UPDATE_DL:\s*/i, '').trim());

  const playerMatch = body.match(/^@?(.+?)\s+(\S+)\s+(.+)$/);
  if (!playerMatch) {
    console.warn(`[parser] DL: could not extract name/hero/rest from: "${body.slice(0, 100)}"`);
    return null;
  }
  ...
  return { game: 'DEADLOCK', playerName, hero, rankCurrent, tierCurrent, currentValue, date: parseDate(dateRaw), dateRaw: dateRaw.trim() };
}
